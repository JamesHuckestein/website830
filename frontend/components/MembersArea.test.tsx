...
describe("MembersArea", () => {
  it("renders all 7 sub-section links", () => {
    render(<MembersArea isOfficer={false} isAdmin={false} officerPosition={null} onSelect={vi.fn()} onLogout={vi.fn()} />);
  ...
  it("calls onSelect with correct id when a link is clicked", () => {
    ...
    render(<MembersArea isOfficer={false} isAdmin={false} officerPosition={null} onSelect={onSelect} onLogout={vi.fn()} />);
  ...
  it("calls onLogout when Logout is clicked", () => {
    ...
    render(<MembersArea isOfficer={false} isAdmin={false} officerPosition={null} onSelect={vi.fn()} onLogout={onLogout} />);
  ...
  it("shows the Calendar Updates link for officers", () => {
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Grand Knight" onSelect={vi.fn()} onLogout={vi.fn()} />);
  ...
  it("dispatches calendarUpdates when the officer link is clicked", () => {
    const onSelect = vi.fn();
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Grand Knight" onSelect={onSelect} onLogout={vi.fn()} />);
  ...
  it("shows Update Officers for Deputy Grand Knight", () => {
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Deputy Grand Knight" onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.getByRole("button", { name: "Update Officers" })).toBeInTheDocument();
  });

  it("shows Update Officers for Recorder", () => {
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Recorder" onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.getByRole("button", { name: "Update Officers" })).toBeInTheDocument();
  });

  it("shows Update Officers for Financial Secretary", () => {
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Financial Secretary" onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.getByRole("button", { name: "Update Officers" })).toBeInTheDocument();
  });

  it("hides Update Officers for Chancellor (non-privileged officer)", () => {
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Chancellor" onSelect={vi.fn()} onLogout={vi.fn()} />);
  ...
  it("shows officer-only links for Admin", () => {
    render(<MembersArea isOfficer={true} isAdmin={true} officerPosition={null} onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.getByRole("button", { name: "Calendar Updates" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Edit Announcements" })).toBeInTheDocument();
    expect(screen.getByRole("button", { name: "Edit Photo Gallery" })).toBeInTheDocument();
  });

  it("shows privileged links for Admin", () => {
    render(<MembersArea isOfficer={true} isAdmin={true} officerPosition={null} onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.getByRole("button", { name: "Update Officers" })).toBeInTheDocument();
  });

  it("shows Change Admin Password link only for Admin", () => {
    render(<MembersArea isOfficer={true} isAdmin={true} officerPosition={null} onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.getByRole("button", { name: "Change Admin Password" })).toBeInTheDocument();
  });

  it("hides Change Admin Password link for non-Admin privileged officer", () => {
    render(<MembersArea isOfficer={true} isAdmin={false} officerPosition="Grand Knight" onSelect={vi.fn()} onLogout={vi.fn()} />);
    expect(screen.queryByRole("button", { name: "Change Admin Password" })).toBeNull();
  });

  it("dispatches adminPassword when the Change Admin Password link is clicked", () => {
    const onSelect = vi.fn();
    render(<MembersArea isOfficer={true} isAdmin={true} officerPosition={null} onSelect={onSelect} onLogout={vi.fn()} />);
    fireEvent.click(screen.getByRole("button", { name: "Change Admin Password" }));
    expect(onSelect).toHaveBeenCalledWith("adminPassword");
  });
});
